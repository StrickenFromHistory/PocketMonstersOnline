...
public class ReadPokemon {
	...
	public ReadPokemon(HashMap<Integer,PokemonBean> pokeList) {
...
//							icontents.setM_attributes(attrList);
	...
	public PokemonBean getPokemon(String name) {
		...
		for(int w = 1; w < 500;w++){
			try{
				pokemon = pokeList.get(w);
				if(pokemon.getName().replaceAll(" ","").equalsIgnoreCase(name.replaceAll(" ",""))){
					return pokemon;
				}
	...
	public PokemonBean getPokemon(int id) {
		PokemonBean pokemon = new PokemonBean();
		for(int w = 1; w < 500;w++){
			try{
				pokemon = pokeList.get(w);
